...
public class MatrixMulti {
	...
	public static class MMMapper extends Mapper<Object, Text, Text, Text>{
		...
		protected void setup(Context context) throws IOException, InterruptedException {
			...
			Matrix1name = config.get("Matrix1name", "A"); //저장해놓은게 없다면 default 값으로 A
		...
		public void map(Object key, Text value, Context context
			...
			Text valpair = new Text();
			
			if(mat.equals(Matrix1name)){
				valpair.set(""+col+" "+v);
				for(int i =0; i< m; i++){ //B행렬의 열만큼 반복해서 써야하므로
					String p = "" + row + "," + i;
					keypair.set(p);
				...
				valpair.set(""+ row +" "+v); //reduce()에서 valuelist끼리 곱해질때 행*열 되어야하므로
				for(int i =0; i< n; i++){ //A 행렬의 행만큼 반복해서 써야하므로
					String p = "" + i + "," + col;
					keypair.set(p);
					context.write(keypair,valpair);
				}
	...
	public static class MMReducer extends Reducer<Text, Text, Text, Text> {
		...
		public void reduce(Text key, Iterable<Text> values, Context context) 
			throws IOException, InterruptedException {
				// valuelist 곱하지 않고 그냥 pair로 내보내기만
				for(Text t: values)
					context.write(key,t);
				}
	}
