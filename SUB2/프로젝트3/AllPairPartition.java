...
public class AllPairPartition {
    public static class MapClass1 extends Mapper<Object, Text, Text, Text> {
        ...
        public void map(Object key, Text value, Context context)
            ...
            boolean isR = "r".equals(tuple[0]);

            if(isR) {
                for (int i = 0; i < numberOfPartitions; i++) {
                    context.write(new Text("(" + partitionId +"," + i + ")"), value);
                }
                return;
            }
            for (int i = 0; i < numberOfPartitions; i++) {
                context.write(new Text("(" + i +"," + partitionId + ")"), value);
            }
        }
    }
    ...
    public static void main(String[] args) throws IOException,
        ...
        job.setMapperClass(AllPairPartition.MapClass1.class);
        job.setReducerClass(AllPairPartition.ReduceClass1.class);
